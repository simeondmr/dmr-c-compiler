...
impl ResolveVarExprLabel for ExprNode {
    fn resolve(&mut self, var_map: &mut HashMap<String, u32>, label_map: &mut HashMap<String, u32>) -> Result<(), CompilerErrors> {
        match self {
            ...
            ExprNode::Unary { unary_operator: _, expr } => expr.resolve(var_map, label_map),
            ExprNode::Binary { binary_operator: _, left_expr, right_expr } => {
                left_expr.resolve(var_map, label_map)?;
                right_expr.resolve(var_map, label_map)
            },
            ExprNode::Assignment { assignment_type: _, dest, expr } => {
                ...
                dest.resolve(var_map, label_map)?;
                expr.resolve(var_map, label_map)
            },
            ExprNode::PrePostOperator { pre_post_operator_type: _, identifier} => identifier.resolve(var_map, label_map),
            ExprNode::Conditional { condition, true_expr, false_expr } => {
                condition.resolve(var_map, label_map)?;
                true_expr.resolve(var_map, label_map)?;
                false_expr.resolve(var_map, label_map)
            },
